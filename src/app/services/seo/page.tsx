    ...
    return (
        <section className="bg-lightBg py-20 px-6 md:py-32 md:px-24">
            <div className="max-w-6xl mx-auto text-center">
                ...
                {/* Секція з SEO послугами */}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-16 lg:gap-24">
                    {seoServices.map((service, index) => (
                        <div
                            key={index}
                            className="relative flex flex-col items-center bg-white p-8 rounded-lg shadow-xl space-y-6"
                            style={{
                                position: "relative",
                                zIndex: "0",
                                transform: `translate(${Math.random() * 10}px, ${Math.random() * 10}px) rotate(${Math.random() * 15 - 7}deg)`,
                            }}
                        >
